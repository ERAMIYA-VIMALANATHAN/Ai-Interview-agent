...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      const { data, error } = await supabase.from('daily_tasks').select('*').eq('user_id', targetId).order('created_at', { ascending: false });
    ...
    if (req.method === 'PUT') {
      ...
      const { data, error } = await supabase.from('daily_tasks').update({ completed, title, updated_at: new Date().toISOString() }).eq('id', id).select().single();
      if (error) throw error;
      return res.status(200).json(data);
    }
